...
def extractdata(url):
   ...
   #put json string into a list
    ...
    for i in range(0, len(lista)):
        name = lista[i]['shop_nombre'].replace('\t','')
        try:
          city = lista[i]['pueb_codigo_nombre']
        except KeyError:
          city = np.nan 
        province = lista[i]['prov_codigo_nombre']
        address = lista[i]['shop_direccion'].replace('\t','')
        try:
...
jsonFile = open("nacex_nxshop_data.json", "w")
jsonFile.write(jsonStr)
jsonFile.close()  

nacex = pd.read_json("nacex_nxshop_data.json")
...
nacex_gdf.to_file("nacex_nxshops.geojson", driver="GeoJSON")